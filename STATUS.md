...
**Headline.** Smoke gate at **99.0% clean** on a deterministic
1000-routine VistA sample. All grammar work for v1.0 scope is
...
- **Real-source smoke gate:** `tools/smoke-corpus.js`, 1000-routine
  deterministic VistA sample, 99.0% clean. Run with
  `node tools/smoke-corpus.js ~/vista-meta/vista/vista-m-host/Packages --sample 1000`.
...
10. **Vendor symbols missing in m-standard.** `ZW` abbreviation
    (ZWRITE), `$ZU` (some forms not in current export), `$ZCALL`
    — these need fixes in `m-standard`'s extractors, not in
    m-parser. Worth filing a tracking note upstream.
